...
Regenerating every file on any change is cheap, but writing every output is not: a rewritten file gets a new timestamp even when its content is identical, which triggers a recompile of it and everything after it, and for `MyriadInlineGeneration` rewrites the very source file being edited - including from design-time builds, so editors see it change on disk after unrelated edits. `Program.fs` therefore compares each output with what is already on disk (`Generation.fileHasLines`) and skips the write when they match. The comparison is line by line, so line endings and a byte order mark do not count as changes. For inline generation the compared content is the input file's own lines plus the generated section, so an edit elsewhere in the input file that leaves the generated code unchanged leaves the file alone, while an edit to an attributed type rewrites it. Skipping the write is safe for the rebuild check above, which is keyed on the stamp file (touched after every run), not on the generated files' timestamps.
