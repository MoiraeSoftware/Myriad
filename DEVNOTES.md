...
The MSBuild implementation relies on `_MyriadSdkCodeGenInputCache` property which is defined in [Myriad.Sdk.targets](./src/Myriad.Sdk/build/Myriad.Sdk.targets) - one combined hash over every codegen input, not a per-file hash. Because of that, `MyriadSdkGenerateCode`'s own up-to-date check is a single stamp file (`_MyriadSdkGenerateStamp`) covering the whole project, not one output-file check per attributed file: editing any one attributed file already invalidates the combined hash and forces every file to regenerate, so a per-file check would only have been misleading, not actually finer-grained. Confirmed by editing a single file and observing every generated file get regenerated, not just the one that depended on it (see below for why only changed output is then written).

## Unchanged output is not written

Regenerating every file on any change is cheap, but writing every output is not: a rewritten file gets a new timestamp even when its content is identical, so editors and FSAC see it change on disk, and for `MyriadInlineGeneration` that file is the very source file being edited - including from design-time builds, so editors see it change on disk after unrelated edits. `Program.fs` therefore compares each output with what is already on disk (`Generation.fileHasLines`) and skips the write when they match. The comparison is line by line, so line endings, a byte order mark and final newlines do not count as changes, and the generated code is written without a trailing blank line, so an editor trimming final newlines on save does not make it differ. (It does not save a compile: Myriad only runs when one of its inputs changed, which is normally itself in `@(Compile)`, so `fsc` reruns regardless.) For inline generation the compared content is the input file's own lines plus the generated section, so an edit elsewhere in the input file that leaves the generated code unchanged leaves the file alone, while an edit to an attributed type rewrites it. Skipping the write is safe for the rebuild check above, which is keyed on the stamp file (touched after every run), not on the generated files' timestamps.
