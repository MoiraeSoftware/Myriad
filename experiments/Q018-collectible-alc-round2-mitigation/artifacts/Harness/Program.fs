...
let satelliteDesignTimeDll = Path.Combine(SPIKE, "SatelliteTP.DesignTime", "bin", "Release", "net8.0", "SatelliteTP.DesignTime.dll")
...
// Q018 Round C: reach into the SAME loaded SatelliteTP.DesignTime.dll instance FCS's own provider
// hosting already loaded (Assembly.LoadFrom's own path-based caching returns the identical instance for
// an identical path, matching this repo's own already-established LoadFrom identity behavior -- verified
// below by checking the call doesn't throw and by observing its effect on the subsequent overwrite,
// rather than assumed), and call its public Q018Hooks.evictAndForceCollect to release Satellite.dll's
// lock BEFORE SatelliteBuilder attempts to overwrite it -- sidestepping the FileSystemWatcher-based
// design's own chicken-and-egg problem (a change-triggered watcher cannot release a lock that blocks the
// very change it's waiting for).
let tryEvictSatelliteViaHook (dllPath: string) : bool option =
    try
        let designTimeAsm = Assembly.LoadFrom satelliteDesignTimeDll
        let hooksType = designTimeAsm.GetType("SatelliteTPImplementation.Q018Hooks")
        if hooksType = null then
            printfn "  Q018Hooks type not found on the loaded design-time assembly"
            None
        else
            let mi = hooksType.GetMethod("evictAndForceCollect", BindingFlags.Public ||| BindingFlags.Static)
            if mi = null then
                printfn "  evictAndForceCollect method not found"
                None
            else
                let result = mi.Invoke(null, [| box dllPath |]) :?> bool
                Some result
    with ex ->
        printfn "  tryEvictSatelliteViaHook threw: %s: %s" (ex.GetType().Name) ex.Message
        None

let round2WithAlcEviction () =
    printfn "\n===== ROUND C (Q018): ALC-based loading + explicit Harness-triggered eviction before overwrite ====="
    printfn "-- step 1: cold check, v1 satellite (2 fields) already loaded from Round 1 in this process --"
    let (_, errsV1, _) = compileConsumer "Round C step1 (v1 sanity)" "ConsumerCPre" (consumerText "ConsumerCPre" false)
    ...
    printfn "-- step 2.5 (Q018): explicitly evict Satellite.dll's collectible ALC BEFORE the overwrite attempt --"
    let satPathForHook = Path.GetFullPath(Path.Combine(sampleLibDir, "Satellite.dll"))
    let evictResult = tryEvictSatelliteViaHook satPathForHook
    printfn "  eviction hook result: %A" evictResult

    printfn "-- step 3: recompile satellite to the SAME path (overwrite) --"
    ...
    let (ms5, errs5, outDll5) = compileConsumer "Round C step5 (post-regen, GetEmail)" "ConsumerCPost" (consumerText "ConsumerCPost" true)
    let step5Ok = errs5.Length = 0
    if step5Ok then
        let asm = Assembly.LoadFrom outDll5
        let t = asm.GetType("ConsumerCPost")
    ...
    printfn "ROUND C verdict: %s (email-getter-generated=%b no-lock-exception=%b post-regen-resolves=%b)"
...
let main argv =
    ...
    | [| "roundC" |] -> if round2WithAlcEviction () then 0 else 1
    | [| "all" |] ->
        let r1 = round1 ()
        let r2 = round2 ()
