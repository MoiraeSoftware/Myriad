// Q028 - JIT/FCS-init tax spot-check on Q023/Q024's own scale-cost numbers (BACKLOG.md item 23),
// a byte-for-byte copy of Q023's own spike (unchanged when warmup is not requested) plus one added
// throwaway warmup check.
//
// Original Q023 header, unchanged below: resolves Q021's own review Objection 1 / Follow-up 1: does
// ParseAndCheckProject cost on an UNCHANGED remainder stay flat as project size N grows (explanation
// (c), real caching) or scale with N regardless of what changed (explanation (b), unconditional full
// recompute)?
//
...
// Q028 addition: an optional throwaway warmup check, run on a tiny, unrelated 1-file project with a
// PLAIN (non-reentrant, non-virtual) FSharpChecker, before the real N-file measurement begins. This
// pays the one-time JIT/FCS-static-init/referenced-assembly-metadata-read tax Q027's review found
// dominates any first ParseAndCheckFileInProject/ParseAndCheckProject call in a fresh process, so it
// lands on the throwaway warmup call instead of on this spike's own "cold" measurement. Everything
// below this comment is new; everything above is Q023's own spike, unmodified.
let runWarmup () =
    let warmupPath = @"C:\virt\Warmup0000.fs"
    let src = "module Warmup0000\ntype T = { X: int }\nlet f (t: T) = t.X + 1\n"
    // Needs its own virtual DocumentSource, same as the main spike's checker - a plain checker with
    // no custom source tries to read warmupPath from the real filesystem and fails.
    let docSource = DocumentSource.Custom(fun path ->
        async { return if path = warmupPath then Some(SourceText.ofString src :> ISourceText) else None })
    let checker = FSharpChecker.Create(keepAssemblyContents = true, documentSource = docSource, useTransparentCompiler = true)
    let baseOptsResult =
        checker.GetProjectOptionsFromScript(warmupPath, SourceText.ofString src)
        |> Async.RunSynchronously
    let baseOpts, _ = baseOptsResult
    let opts = { baseOpts with ProjectFileName = @"C:\virt\warmup.fsproj"; SourceFiles = [| warmupPath |] }
    let sw = Stopwatch.StartNew()
    let result = checker.ParseAndCheckProject(opts) |> Async.RunSynchronously
    sw.Stop()
    let errs = result.Diagnostics |> Array.filter (fun d -> d.Severity = FSharpDiagnosticSeverity.Error)
    printfn "  [warmup] throwaway 1-file check: %5dms  errs=%d" sw.ElapsedMilliseconds errs.Length
...
let main argv =
    ...
    // Third, optional argv (Q028): "warmup" as a literal trailing token enables the throwaway warmup
    // check above. Absent (the default) reproduces Q023's own numbers exactly, unmodified.
    let n, editIdx, warmup =
        match argv with
        | [| nStr |] -> int nStr, 0, false
        | [| nStr; editIdxStr |] -> int nStr, int editIdxStr, false
        | [| nStr; editIdxStr; "warmup" |] -> int nStr, int editIdxStr, true
        | _ -> failwith "usage: q028-spike <N> [editIdx] [warmup]"

    printfn "Q028 - JIT/FCS-init tax spot-check on Q023's own scale-cost spike (BACKLOG.md item 23)"
    printfn "Single-N-per-process run: N=%d editIdx=%d warmup=%b" n editIdx warmup
    printfn ""

    if warmup then runWarmup ()

    let r = runForN editIdx n

    printfn "=== Result (N=%d, editIdx=%d, warmup=%b) ===" n editIdx warmup
