...
let logPath = Path.Combine(__SOURCE_DIRECTORY__, sprintf "lsp-client-adaptive.%s.%s.log" (Path.GetFileName(projectDir)) mode)
...
psi.Arguments <- "tool run fsautocomplete -- --adaptive-lsp-server-enabled"
