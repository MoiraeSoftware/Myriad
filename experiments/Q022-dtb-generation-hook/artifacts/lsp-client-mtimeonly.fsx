...
let logPath = Path.Combine(__SOURCE_DIRECTORY__, sprintf "lsp-client-mtimeonly.%s.%s.log" (Path.GetFileName(projectDir)) mode)
    ...
    if not emailOk then
        ...
        if not emailOk2 then
            ...
            if not emailOk3 then
                ...
                logLine "=== Round 2e-REVIEW: mtime-only bump of .fsproj (no content change, no didChangeWatchedFiles), then workspaceLoad ==="
                System.IO.File.SetLastWriteTimeUtc(fsprojPath, System.DateTime.UtcNow)
                drainUntilQuiet 2000 30000
