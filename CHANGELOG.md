...
### Fixed
- Unchanged generated output is no longer written. Myriad regenerates every file whenever any attributed file changes, and previously rewrote every output even when its content was identical, giving it a new timestamp. For `MyriadInlineGeneration` that meant rewriting the source file being edited on every regeneration, including in design-time builds, so editors saw it change on disk after unrelated edits; separate output files likewise changed on disk for editors and FSAC without any change in content. Output is compared line by line with the existing file, so line endings, a byte order mark and final newlines do not count as changes.
- Generated output no longer ends in a blank line. Fantomas output already ends in a newline, to which writing added another, so an editor trimming final newlines on save made the file differ from the generated output, and be rewritten, on every regeneration. Existing generated files are rewritten once without it.
